...
func Run(c schedulerserverconfig.CompletedConfig, stopCh <-chan struct{}) error {
	...
	// Start up nsnodeselector server.
	if c.ComponentConfig.ServerAddr != "" {
		schedulerConfig := &SchedulerConfig{configer: sched.GetSchdulerConfiger(), client: c.Client, configMapTimeOut: 5 * time.Second}
		StartPolicyHttpServer(c.ComponentConfig.BaseAuthCVSFile, schedulerConfig, c.ComponentConfig.ServerAddr, c.ComponentConfig.ServerCertPath, c.ComponentConfig.ServerKeyPath)
	} else {
		glog.Infof("ServerAddr is empty not start nsnodeselector server")
	}
