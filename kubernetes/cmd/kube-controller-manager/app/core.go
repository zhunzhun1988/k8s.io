...
func startNodePVController(ctx ControllerContext) (bool, error) {
	go nodepvcontroller.NewNodePVController(
		ctx.InformerFactory.Core().V1().Nodes(),
		ctx.InformerFactory.Core().V1().PersistentVolumes(),
...
func startFailPodScalerController(ctx ControllerContext) (bool, error) {
	if ctx.ComponentConfig.FPSController.FailPodScaleLimit <= 0 { // if FailPodScaleLimit <=0 FailPodScalerController will be disabled
		return true, nil
	}
	...
	go failpodscaler.NewFailPodScalerController(
		...
		fpsClientGoClient,
		ctx.ComponentConfig.FPSController.FailPodScaleLimit,
	).Run(ctx.Stop)
	return true, nil
}
