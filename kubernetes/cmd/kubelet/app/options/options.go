...
func NewKubeletFlags() *KubeletFlags {
	...
	return &KubeletFlags{
		...
		ExperimentalXFSDiskMountPath: "/xfs",
		ClusterName:                  "Helium",
		ImageGCWhiteListConfig:       "",
	}
}
...
func NewKubeletServer() (*KubeletServer, error) {
	...
	config.ExperimentalXFSDiskMountPath = "/xfs"
	config.ClusterName = "Helium"
...
func (f *KubeletFlags) AddFlags(mainfs *pflag.FlagSet) {
	...
	fs.StringVar(&f.ExperimentalXFSDiskMountPath, "experimental-quota-xfs-disk-path", f.ExperimentalXFSDiskMountPath, "[Experimental] check the projquota xfs mount path for hostpath pv quota")
	fs.StringVar(&f.ClusterName, "clusterName", f.ClusterName, "which cluster node belong to")
	fs.StringVar(&f.ImageGCWhiteListConfig, "image-gc-whitelist-config", f.ImageGCWhiteListConfig, "Path to a whitelist file that will be used to skip some special images for garbage collection. Ex . '/etc/kubernetes/imagewhitelist.json'.[default='']")
}
