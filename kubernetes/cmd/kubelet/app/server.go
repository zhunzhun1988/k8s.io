...
type CadvisorSyncIntervalGet struct {
	client       clientset.Interface
	lastSyncTime time.Time
	lastInterval time.Duration
}

func (csig *CadvisorSyncIntervalGet) GetInterval() time.Duration {
	defaultInterval := 10 * time.Second
	minInterval := 10 * time.Second
	maxInterval := 3 * time.Minute
	randInterval := func(d time.Duration) time.Duration {
		//		ret := d + time.Duration(rand.Float64()*float64(d))
		if d < minInterval {
			d = minInterval
	...
	if time.Since(csig.lastSyncTime) < 10*time.Second {
		return randInterval(csig.lastInterval)
	}
	cm, err := csig.client.Core().ConfigMaps("kube-system").Get("cadvisorconfig", metav1.GetOptions{})
	if err == nil || cm.Data[configInterval] != "" {
		buf := cm.Data[configInterval]
		duration, errAtoi := strconv.Atoi(buf)
		if errAtoi == nil {
			csig.lastInterval = time.Duration(duration) * time.Second
			csig.lastSyncTime = time.Now()
			return randInterval(csig.lastInterval)
		}
	}
	csig.lastSyncTime = time.Now()
	csig.lastInterval = defaultInterval
	return randInterval(csig.lastInterval)
}
...
func run(s *options.KubeletServer, kubeDeps *kubelet.Dependencies, stopCh <-chan struct{}) (err error) {
	...
	if kubeDeps.CAdvisorInterface == nil {
		...
		kubeDeps.CAdvisorInterface, err = cadvisor.New(s.HostnameOverride, s.KubeletFlags.ClusterName,
			s.KubeletFlags.ExperimentalXFSDiskMountPath, &CadvisorSyncIntervalGet{client: kubeDeps.KubeClient},
			s.Address, uint(s.CAdvisorPort), imageFsInfoProvider, s.RootDirectory, cadvisor.UsingLegacyCadvisorStats(s.ContainerRuntime, s.RemoteRuntimeEndpoint))
		if err != nil {
			return err
		}
	}
...
func RunKubelet(kubeFlags *options.KubeletFlags, kubeCfg *kubeletconfiginternal.KubeletConfiguration, kubeDeps *kubelet.Dependencies, runOnce bool) error {
	...
	kubeCfg.ExperimentalXFSDiskMountPath = kubeFlags.ExperimentalXFSDiskMountPath
