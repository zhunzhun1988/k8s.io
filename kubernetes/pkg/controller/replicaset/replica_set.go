...
const (
	...
	// Is the ReplicaSet rescheduling now
	RSRescheduing = "io.enndata.rescheduler/rescheduling"
)
...
func (rsc *ReplicaSetController) isRsRescheduling(rs *apps.ReplicaSet) bool {
	if rs.Annotations != nil {
		str := "false"
		str = rs.Annotations[RSRescheduing]
		if str == "true" {
			return true
		}
	}
...
func (rsc *ReplicaSetController) syncReplicaSet(key string) error {
	...
	if rsc.isRsRescheduling(rs) == true {
		glog.Infof("rs %v is rescheduling resync after 10 seconds", rs)
		rsc.queue.AddAfter(key, 10*time.Second)
		return nil
	}
