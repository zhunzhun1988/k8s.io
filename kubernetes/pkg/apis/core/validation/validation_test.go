...
func TestValidatePodDNSConfig(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc: "invalid: 9 search paths",
			dnsConfig: &core.PodDNSConfig{
				Searches: []string{"custom", "mydomain.com", "local", "cluster.local", "svc.cluster.local", "ns.pod.cluster.local", "pod.cluster.local", "default.svc.cluster.local", "exceeded"},
			},
			expectedError: true,
		},
		{
