...
func (c *Cloud) GetServicePortFromTag(loadBalancer *elbv2.LoadBalancer, namespacedName types.NamespacedName) (currentServiceFrontedPortMap, currentServiceTargetPortMap map[int64]bool, err error) {
	tagsDescription, err := c.elbv2.DescribeTags(
		&elbv2.DescribeTagsInput{
			ResourceArns: []*string{loadBalancer.LoadBalancerArn},
		},
	)
	servicePortTag := ""
	for _, serviceTag := range tagsDescription.TagDescriptions[0].Tags {
		if strings.EqualFold(*serviceTag.Key, namespacedName.String()) {
			servicePortTag = *serviceTag.Value
		}
	}

	currentServiceFrontedPortMap = map[int64]bool{}
	currentServiceTargetPortMap = map[int64]bool{}
	portSets := strings.Split(servicePortTag, " ")
	if len(portSets) == 0 {
		return currentServiceFrontedPortMap, currentServiceTargetPortMap, nil
	}

	for _, tagPort := range portSets {
		frontedTargetPort := strings.Split(tagPort, "/")
		if len(frontedTargetPort) != 2 {
			continue
		}
		frontedPort, err := strconv.ParseInt(frontedTargetPort[0], 10, 64)
		if err != nil {
			return nil, nil, fmt.Errorf("Error in parse service port from tag")
		}

		targetPort, err := strconv.ParseInt(frontedTargetPort[1], 10, 64)
		if err != nil {
			return nil, nil, fmt.Errorf("Error in parse service port from tag")
		}

		currentServiceFrontedPortMap[frontedPort] = true
		currentServiceTargetPortMap[targetPort] = true
...
func (c *Cloud) ensureLoadBalancerv2(namespacedName types.NamespacedName, loadBalancerName string, mappings []nlbPortMapping, instanceIDs, subnetIDs []string, internalELB bool, annotations map[string]string) (*elbv2.LoadBalancer, error) {
	...
	//Add service Port tag
	servicePortTag := ""
	for _, tagPort := range mappings {
		servicePortTag = fmt.Sprintf("%s %s/%s", servicePortTag, strconv.FormatInt(tagPort.FrontendPort, 10), strconv.FormatInt(tagPort.TrafficPort, 10))
	}

	tags[namespacedName.String()] = servicePortTag
	...
	if loadBalancer == nil {
		...
		// sync mappings
		{
			...
			//find the service port in tag
			currentServiceFrontedPortMap, currentServiceTargetPortMap, err := c.GetServicePortFromTag(loadBalancer, namespacedName)
			if err != nil {
				return nil, fmt.Errorf("Error get service port from loadbalancer tag: %q", err)
			}
			...
			for _, listener := range listenerDescriptions.Listeners {
				if _, ok := currentServiceFrontedPortMap[*listener.Port]; ok {
					actual[*listener.Port] = listener
				}
			}
			glog.V(2).Infof("get from aws loadbalacer actual map (%v)", actual)
			...
			for _, targetGroup := range actualTargetGroups.TargetGroups {
				if _, ok := currentServiceTargetPortMap[*targetGroup.Port]; ok {
					nodePortTargetGroup[*targetGroup.Port] = targetGroup
				}
			}
			glog.V(2).Infof("get from aws nodePortTargetGroup:(%v)", nodePortTargetGroup)
			...
			for _, mapping := range mappings {
				...
				glog.V(2).Infof("Handle additions/modifications,Port:(%v)", frontendPort)
			...
			// Add tags to new targets,but delete tags first

			addTagsInput.ResourceArns = append(addTagsInput.ResourceArns, loadBalancer.LoadBalancerArn)

			//delete tags one by one because of aws limits
			for _, removeTagsArn := range addTagsInput.ResourceArns {
				_, err = c.elbv2.RemoveTags(
					&elbv2.RemoveTagsInput{
						ResourceArns: []*string{removeTagsArn},
						TagKeys: []*string{
							aws.String(namespacedName.String()),
						},
					},
				)
				if err != nil {
					return nil, fmt.Errorf("error delete tag  after modifying load balancer targets: %q", err)
				}
				glog.V(2).Infof("delete tags to (%v), key is (%s)", loadBalancer.LoadBalancerName, namespacedName.String())
			}
			...
			//delete tags one by one because of aws limits
			if len(addTagsInput.ResourceArns) > 0 && len(addTagsInput.Tags) > 0 {
				for _, addTagsArn := range addTagsInput.ResourceArns {
					_, err := c.elbv2.AddTags(
						&elbv2.AddTagsInput{
							ResourceArns: []*string{addTagsArn},
							Tags:         addTagsInput.Tags,
						},
					)
					if err != nil {
			...
			glog.V(2).Infof("add tags to (%v)", loadBalancer.LoadBalancerName)
		}
...
func (c *Cloud) updateInstanceSecurityGroupsForNLBTraffic(actualGroups []*ec2.SecurityGroup, desiredSgIds []string, ports []int64, lbName string, serviceName types.NamespacedName, originTagPortMap map[int64]bool, clientCidrs []string, clientTraffic bool) error {
	...
	for _, actualGroup := range actualGroups {
		...
		if ok {
			...
			//filter port with service tag
			for existingPort := range existingSet {
				_, ok := originTagPortMap[existingPort]

				if !ok {
					existingSet.Delete(existingPort)
				}
			}
...
func (c *Cloud) updateInstanceSecurityGroupsForNLB(mappings []nlbPortMapping, instances map[awsInstanceID]*ec2.Instance, lbName string, serviceName types.NamespacedName, originTagPortMap map[int64]bool, clientCidrs []string) error {
	...
	err = c.updateInstanceSecurityGroupsForNLBTraffic(actualGroups, desiredGroupIds, trafficPorts, lbName, serviceName, originTagPortMap, clientCidrs, true)
	if err != nil {
		return err
	}

	// Run once for health check traffic
	err = c.updateInstanceSecurityGroupsForNLBTraffic(actualGroups, desiredGroupIds, healthCheckPorts, lbName, serviceName, originTagPortMap, []string{aws.StringValue(vpcCidr)}, false)
	if err != nil {
		return err
	}
