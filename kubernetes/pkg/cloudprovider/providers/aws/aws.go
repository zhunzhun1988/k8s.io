...
type ELBV2 interface {
	...
	RemoveTags(input *elbv2.RemoveTagsInput) (*elbv2.RemoveTagsOutput, error)
...
func (c *Cloud) EnsureLoadBalancer(ctx context.Context, clusterName string, apiService *v1.Service, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	if isNLB(annotations) {
		...
		glog.V(2).Infof("loadbalancer name is %s", loadBalancerName)
		...
		originTagPortMap := map[int64]bool{}
		lb, err := c.describeLoadBalancerv2(loadBalancerName)
		if err != nil {
			return nil, err
		}
		if lb != nil {
			glog.V(2).Infof("loadbalancer named %s has created before ", loadBalancerName)
			_, originTagPortMap, err = c.GetServicePortFromTag(lb, serviceName)
			if err != nil {
		...
		err = c.updateInstanceSecurityGroupsForNLB(v2Mappings, instances, loadBalancerName, serviceName, originTagPortMap, sourceRangeCidrs)
		if err != nil {
...
func (c *Cloud) EnsureLoadBalancerDeleted(ctx context.Context, clusterName string, service *v1.Service) error {
	...
	if isNLB(service.Annotations) {
		...
		// * Delete Load Balancer's listeners
		// * Delete target groups
		// * Clean up SecurityGroupRules
		namespacedName := types.NamespacedName{Namespace: service.Namespace, Name: service.Name}
		//find the service port in tag
		currentServiceFrontedPortMap, currentServiceTargetPortMap, err := c.GetServicePortFromTag(lb, namespacedName)
		if err != nil {
			return fmt.Errorf("Error get service port from loadbalancer tag: %q", err)
		}
		glog.V(2).Infof("currentServiceFrontedPortMap: %v", currentServiceFrontedPortMap)
		glog.V(2).Infof("currentServiceTargetPortMap: %v", currentServiceTargetPortMap)

		{
			...
			if err != nil {
				return fmt.Errorf("Error describing listeners: %q", err)
			}

			actualListenerNum := len(listenerDescriptions.Listeners)
			deleteListenerNum := 0
			...
			for _, listener := range listenerDescriptions.Listeners {
				if _, ok := currentServiceFrontedPortMap[*listener.Port]; ok {
					...
					if err != nil {
						return fmt.Errorf("Error deleting listener :%q", err)
					}
					deleteListenerNum++
				}
			}
			//delete the nlb if it has no listeners
			if actualListenerNum == deleteListenerNum && loadBalancerName != "nlb" {
				glog.V(2).Infof("no lieteners in loadbalancer %s, will be deleted", loadBalancerName)
				_, err := c.elbv2.DeleteLoadBalancer(&elbv2.DeleteLoadBalancerInput{
					LoadBalancerArn: lb.LoadBalancerArn,
				})
				if err != nil {
					return fmt.Errorf("Error delete nil loadbalancer: %q", err)
				}
				glog.V(2).Infof("successful delete loadbalancer: %v", lb.LoadBalancerName)
...

		{
...

			{
				...
				for i := range matchingGroups {
					...
					for j := range matchingGroups[i].IpPermissions {
						...
						for k := range matchingGroups[i].IpPermissions[j].IpRanges {
							...
							if description == clientRule || description == mtuRule || description == healthRule {
								FromportNum := matchingGroups[i].IpPermissions[j].FromPort
								//	ToportNum := matchingGroups[i].IpPermissions[j].ToPort
								//	IpRanges := matchingGroups[i].IpPermissions[j].IpRanges
								_, ok := currentServiceTargetPortMap[int64(*FromportNum)]
								if ok {
					...
					//delete all ports belong to the same nlb but not the same service
...

		{
			...
			if err != nil {
				return fmt.Errorf("error delete tag  after delete load balancer service: %q", err)
			}
			glog.V(2).Infof("delete tags to (%v), key is (%s)", lb.LoadBalancerName, namespacedName.String())
...

	{
		...
		for {
			...
			if len(securityGroupIDs) == 0 {
				glog.V(2).Infof("Deleted all security groups for load balancer: %s", service.Name)
				break
			}
			...
			glog.V(2).Infof("Waiting for load-balancer to delete so we can delete security groups: %v", service.Name)
