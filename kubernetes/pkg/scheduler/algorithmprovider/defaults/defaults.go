...
func init() {
	...
	// Fit is determined by node's pods namespace.
	factory.RegisterFitPredicate("NodeNameSpacePredicate", predicates.PodNamespaceSelect)
	//the same namespace pod should placed int the same node
	factory.RegisterPriorityConfigFactory(
		"NodeNameSpacePriority",
		factory.PriorityConfigFactory{
			Function: func(args factory.PluginFactoryArgs) algorithm.PriorityFunction {
				return priorities.NewNameSpacePriority(args.ServiceLister, args.ControllerLister, args.ReplicaSetLister)
			},
			Weight: 1,
		},
	)
...
func defaultPredicates() sets.String {
	return sets.NewString(
		...
		// Fit is node quota disk satisfy the pod host pv path request.
		factory.RegisterFitPredicateFactory(
			"PodHostPathPVDiskPressure",
			func(args factory.PluginFactoryArgs) algorithm.FitPredicate {
				return predicates.NewPodHostPVDiskPredicate(args.PVInfo, args.PVCInfo)
			},
		),

		// node quota disk satisfy the pod host pv path request.
		factory.RegisterFitPredicateFactory(
			"HostPathPVAffinityPredicate",
			func(args factory.PluginFactoryArgs) algorithm.FitPredicate {
				return predicates.NewHostPathPVAffinityPredicate(args.PVInfo, args.PVCInfo, args.PodLister)
			},
		),
		...
		factory.RegisterFitPredicateFactory(
			...
			func(args factory.PluginFactoryArgs) algorithm.FitPredicate {
				return predicates.NewHostPathPVAffinityPredicate2(args.PVInfo, args.PVCInfo, args.PodLister)
			},
		),
		...
		factory.RegisterFitPredicateFactory(
			...
			func(args factory.PluginFactoryArgs) algorithm.FitPredicate {
				return predicates.NewNamespaceNodeSelectorPredicate(args.NsNodeSelectorConfigMap)
			},
		),
	)
}
...
func insert(set sets.String, str string) sets.String {
	result := sets.NewString(set.List()...)
	if result.Has(str) == false {
		result.Insert(str)
	}
	return result
}
...
func registerAlgorithmProvider(predSet, priSet sets.String) {
	...
	factory.RegisterAlgorithmProvider(factory.NameSpacePriorityProvider, predSet, insert(priSet, "NodeNameSpacePriority"))
	factory.RegisterAlgorithmProvider(factory.NameSpacePredicateProvider, insert(priSet, "NodeNameSpacePredicate"), priSet)
}
...
func defaultPriorities() sets.String {
	return sets.NewString(
		...
		factory.RegisterPriorityConfigFactory(
			...
			factory.PriorityConfigFactory{
				MapReduceFunction: func(args factory.PluginFactoryArgs) (algorithm.PriorityMapFunction, algorithm.PriorityReduceFunction) {
					return priorities.NewNodeHostPathSpreadPriority(args.PVInfo, args.PVCInfo)
				},
				Weight: 2,
			},
		),
		factory.RegisterPriorityConfigFactory(
			...
			factory.PriorityConfigFactory{
				MapReduceFunction: func(args factory.PluginFactoryArgs) (algorithm.PriorityMapFunction, algorithm.PriorityReduceFunction) {
					return priorities.NewNodePVDiskUsePriority(args.PVInfo, args.PVCInfo)
				},
				Weight: 1,
			},
		),
