...
func New(hostnameOverride, clusterName, quotadiskpath string, housekeepingIntervalGet manager.HousekeepingIntervalGet, address string, port uint, imageFsInfoProvider ImageFsInfoProvider, rootPath string, usingLegacyStats bool) (Interface, error) {
	...
	backendStorage, err := storage.New(*storageDriver, clusterName)
	if err != nil {
		return nil, err
	}
	...
	m, err := manager.New(hostnameOverride, clusterName, quotadiskpath, housekeepingIntervalGet,
		memory.New(statsCacheDuration, backendStorage), sysFs, maxHousekeepingInterval, allowDynamicHousekeeping, ignoreMetrics, http.DefaultClient)
	if err != nil {
		return nil, err
	}
