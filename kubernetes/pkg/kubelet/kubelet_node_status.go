...
func (kl *Kubelet) setNodeDiskQuotaInfo(node *v1.Node) (changed bool) {
	if kl.xfsQuotaManager == nil {
		return false
	}
	info := make(xfs.NodeDiskQuotaInfoList, 0)
	diskInfo := kl.xfsQuotaManager.GetQuotaDiskInfos()
	diskStatus := kl.xfsQuotaManager.GetQuotaStatus()
	statusBuf, errDiskStatus := json.Marshal(diskStatus)

	if errDiskStatus != nil {
		return false
	}
	...
	if node.Annotations != nil && node.Annotations[xfs.NodeDiskQuotaInfoAnn] == string(buf) &&
		node.Annotations[xfs.NodeDiskQuotaStatusAnn] == string(statusBuf) {
		return false
	}
	if node.Annotations == nil {
		node.Annotations = make(map[string]string)
	}
	node.Annotations[xfs.NodeDiskQuotaInfoAnn] = string(buf)
	node.Annotations[xfs.NodeDiskQuotaStatusAnn] = string(statusBuf)
	return true
}

func (kl *Kubelet) syncNodeDiskQuotaStatus() {
	if err := kl.updateNodeDiskQuotaInfo(); err != nil {
		glog.Errorf("Unable to update node disk quota info: %v", err)
	}
}

func (kl *Kubelet) updateNodeDiskQuotaInfo() error {

	node, err := kl.kubeClient.Core().Nodes().Get(string(kl.nodeName), metav1.GetOptions{})

	if err != nil {
		return fmt.Errorf("error getting node %q: %v", kl.nodeName, err)
	}

	disableList := kl.getNodeQuotadiskDisableList(node)
	disabled := make(map[string]bool)
	for _, disabledisk := range disableList {
		err := kl.xfsQuotaManager.SetQuotaDiskDisabled(disabledisk, true)
		if err != nil {
	...
	for _, disk := range diskInfo {
		if disabled, _ := disabled[path.Clean(disk.MountPath)]; disabled == false {
			err := kl.xfsQuotaManager.SetQuotaDiskDisabled(disk.MountPath, false)
			if err != nil {
				glog.Errorf("set disk %s able fail:%v", disk.MountPath, err)
			}
		}
	}

	changed := kl.setNodeDiskQuotaInfo(node)
	if changed {
		_, errUpdate := kl.kubeClient.Core().Nodes().Update(node)
		if errUpdate != nil {
			return errUpdate
		}
	}
...
func (kl *Kubelet) getNodeQuotadiskDisableList(node *v1.Node) []string {
...
func (kl *Kubelet) getNodeDiskCapacity() int64 {
	if kl.xfsQuotaManager == nil {
		return 0
	}
	var ret int64
