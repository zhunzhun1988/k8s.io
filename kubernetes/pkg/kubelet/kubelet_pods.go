...
func (kl *Kubelet) GenerateRunContainerOptions(pod *v1.Pod, container *v1.Container, podIP string) (*kubecontainer.RunContainerOptions, func(), error) {
	...
	//default to be false, env will be contain in pod;
	filter := false
	if pod.Annotations != nil {
		filterAnno := pod.Annotations["io.enndata.kubelet/podenv.filter"]
		if filterAnno != "" {
			filter, err = strconv.ParseBool(filterAnno)
			if err != nil {
				glog.Errorf("Error on parse io.enndata.kubelet/podenv.filter to bool")
			}
		}
	}
	envs, err := kl.makeEnvironmentVariables(pod, container, podIP, filter)
	if err != nil {
		return nil, nil, err
	}
...
func (kl *Kubelet) getServiceEnvVarMap(ns string, needFilter bool) (map[string]string, error) {
	var (
...
func (kl *Kubelet) makeEnvironmentVariables(pod *v1.Pod, container *v1.Container, podIP string, filterSvcEnv bool) ([]kubecontainer.EnvVar, error) {
	...
	serviceEnv, err := kl.getServiceEnvVarMap(pod.Namespace, filterSvcEnv)
	if err != nil {
		return result, err
	}
