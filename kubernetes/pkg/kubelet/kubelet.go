...
func LoadFromFile(filename string) ([]string, error) {
	...
	whiteListImages, err := Load(configBytes)
	if err != nil {
		return nil, err
	}
	return whiteListImages, err
}

func Load(data []byte) ([]string, error) {
	jsonFormatDate := []WhiteListImage{}
	results := []string{}
	err := json.Unmarshal(data, &jsonFormatDate)
	if err != nil {
		return nil, err
	}
	for _, wli := range jsonFormatDate {
		results = append(results, wli.ID)
	}
	return results, err
}
...
func NewMainKubelet(kubeCfg *kubeletconfiginternal.KubeletConfiguration,
	...
	if kubeCfg.ImageGCWhiteListConfig != "" {
		ImageWhiteList, err := LoadFromFile(kubeCfg.ImageGCWhiteListConfig)
		if err != nil {
			glog.Fatalf("Faild to load ImageGCWhiteListConfig file: %v", err)
			return nil, err
		}
		imageGCPolicy.ImageWhiteList = ImageWhiteList
	}
	...
	if kubeDeps.KubeClient != nil {
		pvLW := cache.NewListWatchFromClient(kubeDeps.KubeClient.CoreV1().RESTClient(), "persistentvolumes", metav1.NamespaceAll, fields.Everything())
		r := cache.NewReflector(pvLW, &v1.PersistentVolume{}, pvIndexer, 0)
		go r.Run(wait.NeverStop)
	}
	...
	klet := &Kubelet{
		...
		xfsQuotaManager:                         xfs.NewXFSQuotaManager(kubeCfg.ExperimentalXFSDiskMountPath, utilexec.New()),
		pvLister:                                pvLister,
	}
	...
	klet.volumeManager = volumemanager.NewVolumeManager(
		...
		klet.xfsQuotaManager,
		klet.pvLister,
		func() bool {
			return klet.sourcesReady.AllReady()
		})
...
func (kl *Kubelet) Run(updates <-chan kubetypes.PodUpdate) {
	...
	kl.xfsQuotaManager.StartQuotaStatusSync()
