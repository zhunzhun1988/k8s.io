...
const (
	...
	pvNodeAffinityUpdatePeriod time.Duration = 20 * time.Second
)
...
func NewVolumeManager(
	...
	isReady pvnodeaffinity.ReadyFun) VolumeManager {
	...
	vm.pvnodeaffinity = pvnodeaffinity.NewPVNodeAffinity(
		kubeClient,
		pvNodeAffinityUpdatePeriod,
		string(nodeName),
		dqm,
		podManager,
		pvLister,
...
type volumeManager struct {
	...
	// record the node mount hostpath pv info for scheduler give a high score to
	// node which had mount the hostpath before
	pvnodeaffinity pvnodeaffinity.PVNodeAffinity
}
...
func (vm *volumeManager) Run(sourcesReady config.SourcesReady, stopCh <-chan struct{}) {
	...
	go vm.pvnodeaffinity.Run(stopCh)
	<-stopCh
