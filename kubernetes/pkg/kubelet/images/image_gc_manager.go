...
func (im *realImageGCManager) Start() {
	go wait.Until(func() {
		...
		_, err := im.detectImages(ts, im.policy.ImageWhiteList)
		if err != nil {
...
func (im *realImageGCManager) detectImages(detectTime time.Time, whitelist []string) (sets.String, error) {
	imagesInUse := sets.NewString()
	for _, whiteListImageID := range whitelist {
		glog.V(5).Infof("Image %s is taged not gc,skip", whiteListImageID)
...
func (im *realImageGCManager) freeSpace(bytesToFree int64, freeTime time.Time) (int64, error) {
	imagesInUse, err := im.detectImages(freeTime, im.policy.ImageWhiteList)
	if err != nil {
		return 0, err
	}
