...
func (c *Configurer) generateSearchesForDNSClusterFirst(hostSearch []string, pod *v1.Pod) []string {
	...
	nsPodDomain := fmt.Sprintf("%s.pod.%s", pod.Namespace, c.ClusterDomain)
	podDomain := fmt.Sprintf("pod.%s", c.ClusterDomain)
	clusterSearch := []string{nsSvcDomain, svcDomain, nsPodDomain, podDomain, c.ClusterDomain}
