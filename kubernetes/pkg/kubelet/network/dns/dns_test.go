...
func TestGetPodDNS(t *testing.T) {
	...
	expLength := len(options[1].DNSSearch) + 5
	if expLength > 8 {
		expLength = 8
	}
...
func TestGetPodDNSCustom(t *testing.T) {
	...
	testNsPodDomain := fmt.Sprintf("%s.pod.%s", testPodNamespace, testClusterDNSDomain)
	testPodDomain := fmt.Sprintf("pod.%s", testClusterDNSDomain)
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedDNSConfig: &runtimeapi.DNSConfig{
				...
				Searches: []string{testNsSvcDomain, testSvcDomain, testNsPodDomain, testPodDomain, testClusterDNSDomain, testHostDomain},
		...
		},
		{
			...
			expectedDNSConfig: &runtimeapi.DNSConfig{
				...
				Searches: []string{testNsSvcDomain, testSvcDomain, testNsPodDomain, testPodDomain, testClusterDNSDomain, testHostDomain, "my.domain"},
				Options:  []string{"ndots:3", "debug"},
			},
		},
		{
	...
	for _, tc := range testCases {
		...
		if !dnsConfigsAreEqual(resDNSConfig, tc.expectedDNSConfig) {
			t.Errorf("%s %v, want %v", tc.desc, resDNSConfig, tc.expectedDNSConfig)
		}
	}
}
