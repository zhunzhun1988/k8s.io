...
func (r *hostPathProvisioner) Provision(selectedNode *v1.Node, allowedTopologies []v1.TopologySelectorTerm) (*v1.PersistentVolume, error) {
	...
	keep := "keep"
	forOnePod := "true"
	if r.options.Parameters != nil && r.options.Parameters["io.enndata.user/alpha-pvhostpathmountpolicy"] != "" {
		keep = r.options.Parameters["io.enndata.user/alpha-pvhostpathmountpolicy"]
	}
	if r.options.Parameters != nil && r.options.Parameters["io.enndata.user/alpha-pvhostpathquotaforonepod"] != "" {
		forOnePod = r.options.Parameters["io.enndata.user/alpha-pvhostpathquotaforonepod"]
	}
	...
	pv := &v1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				"io.enndata.user/alpha-pvhostpathquotaforonepod": forOnePod,
			},
		},
