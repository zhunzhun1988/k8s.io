...
func (plugin *rbdPlugin) getAdminAndSecret(spec *volume.Spec) (string, string, error) {
	...
	admin := ""
	if spec.PersistentVolume.Spec.StorageClassName != "" {
		...
		//PersistentVolume is not auto created by storageclass
		//this only works in enncloud k8s cluster and
		//should disable PersistentVolumeClaimResize admission plugin
		// for all namespaces:
		// console will auto create a user which has same name as namespace with full caps in ceph
		// all console will auto create a secret same name as namespace contains ceph user's keyring
		glog.V(4).Info("PersistentVolume %s is not dynamically provisioned by storageclass",spec.PersistentVolume.Name)
		if spec.PersistentVolume.Spec.RBD.SecretRef == nil {
			return admin,"", fmt.Errorf("spec.PersistentVolume.Spec.RBD.SecretRef is nil")
		}
		adminSecretName = spec.PersistentVolume.Spec.RBD.SecretRef.Name

		if spec.PersistentVolume.Spec.ClaimRef == nil{
			return admin,"", fmt.Errorf("spec.PersistentVolume.Spec.ClaimRef is nil")
		}
		adminSecretNamespace = spec.PersistentVolume.Spec.ClaimRef.Namespace
		admin = adminSecretNamespace
	}
	if admin == "" {
