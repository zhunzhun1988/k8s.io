...
func New(hostnameOverride, clusterName, quotadiskpath string, housekeepingIntervalGet HousekeepingIntervalGet,
	memoryCache *memory.InMemoryCache, sysfs sysfs.SysFs, maxHousekeepingInterval time.Duration, allowDynamicHousekeeping bool, ignoreMetricsSet container.MetricSet, collectorHttpClient *http.Client) (Manager, error) {
	if memoryCache == nil {
	...
	newManager := &manager{
		...
		housekeepingIntervalGet:  housekeepingIntervalGet,
	}
...
func (self *manager) Start() error {
	err := docker.Register(self.hostnameOverride, self, self.fsInfo, self.ignoreMetrics)
	if err != nil {
		glog.V(5).Infof("Registration of the Docker container factory failed: %v.", err)
	}

	err = rkt.Register(self.hostnameOverride, self, self.fsInfo, self.ignoreMetrics)
	if err != nil {
	...
	err = containerd.Register(self.hostnameOverride, self, self.fsInfo, self.ignoreMetrics)
	if err != nil {
	...
	err = systemd.Register(self.hostnameOverride, self, self.fsInfo, self.ignoreMetrics)
	if err != nil {
		glog.V(5).Infof("Registration of the systemd container factory failed: %v", err)
	}

	err = raw.Register(self.hostnameOverride, self.clusterName, self.quotadiskpath, self, self.fsInfo, self.ignoreMetrics)
	if err != nil {
...
func (m *manager) createContainerLocked(containerName string, watchSource watcher.ContainerWatchSource) error {
	...
	cont, err := newContainerData(containerName, m.memoryCache, handler, logUsage, collectorManager, m.maxHousekeepingInterval, m.allowDynamicHousekeeping, clock.RealClock{}, m.housekeepingIntervalGet)
	if err != nil {
		return err
	}
