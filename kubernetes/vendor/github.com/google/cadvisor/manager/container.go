...
type HousekeepingIntervalGet interface {
...
func newContainerData(containerName string, memoryCache *memory.InMemoryCache, handler container.ContainerHandler, logUsage bool, collectorManager collector.CollectorManager, maxHousekeepingInterval time.Duration, allowDynamicHousekeeping bool, clock clock.Clock, housekeepingIntervalGet HousekeepingIntervalGet) (*containerData, error) {
	if memoryCache == nil {
...
func (c *containerData) housekeeping() {
	...
	for {
		...
		houseKeepingTimer.Reset(c.housekeepingIntervalGet.GetInterval())
	}
}
