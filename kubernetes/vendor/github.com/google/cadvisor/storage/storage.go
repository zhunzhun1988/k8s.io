...
type clusterNameStorageDriver struct {
	StorageDriver
	clusterName string
}

func (cnsd *clusterNameStorageDriver) AddStats(cInfo *info.ContainerInfo, stats *info.ContainerStats) error {
	stats.ClusterName = cnsd.clusterName
	cInfo.ClusterName = cnsd.clusterName
	return cnsd.StorageDriver.AddStats(cInfo, stats)
}
...
func New(name string, clusterName string) (StorageDriver, error) {
	if name == "" {
		return nil, nil
	}
	...
	sd, err := f()
	return &clusterNameStorageDriver{
		StorageDriver: sd,
