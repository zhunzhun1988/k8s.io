...
func (driver *kafkaStorage) AddStats(cInfo *info.ContainerInfo, stats *info.ContainerStats) error {
	...
	if driver.producer == nil {
		brokerList := strings.Split(*brokers, ",")
		producer, err := kafka.NewAsyncProducer(brokerList, driver.config)
		if err != nil {
			return err
		}

		driver.producer = producer
	}
	driver.producer.Input() <- &kafka.ProducerMessage{
		Topic: driver.topic,
		Key:   kafka.StringEncoder(detail.HostIp + "-" + detail.ContainerName),
...
func newStorage(machineName string) (storage.StorageDriver, error) {
	...
	// if not set false, retry connection failed
