...
type GpuUsage struct {
	// The serial number of the gpu
	GPUNum uint `json:"gpu_num"`
	...
	// The GPU fan speed
	GPUFan string `json:"gpu_fan"`

	// The GPU temp
	GPUTemp string `json:"gpu_temp"`

	// The GPU power capacity
	GPUPowerCapacity string `json:"gpu_power_capacity"`

	// The GPU power usage
	GPUPowerUsage string `json:"gpu_power_usage"`

	// The GPU memory capacity
	GPUMemoryCapacity string `json:"gpu_memory_capacity"`
...
type GpuStats struct {
	NumOfGPU uint       `json:"numofgpu"`
	Usages   []GpuUsage `json:"usages"`
}

type QuotaStatus struct {
	Capacity      uint64 `json:"capacity"`
	CurUseSize    uint64 `json:"curusesize"`
	CurQuotaSize  uint64 `json:"curquotasize"`
	AvaliableSize string `json:"avaliablesize"`
	MountPath     string `json:"mountpath"`
}

// current disk quota status
type DiskQuotaStats struct {
	NumOfQuotaDisk uint          `json:"numofquotadisk"`
	Capacity       uint64        `json:"capacity"`
	CurUseSize     uint64        `json:"curusesize"`
	CurQuotaSize   uint64        `json:"curquotasize"`
	AvaliableSize  string        `json:"avaliablesize"`
	DiskStatus     []QuotaStatus `json:"diskstatus"`
}
...
type ContainerStats struct {
	...
	DiskQuota   DiskQuotaStats `json:"diskquota,omitempty"`
