...
func (self *rawFactory) NewContainerHandler(name string, inHostNamespace bool) (container.ContainerHandler, error) {
	...
	return newRawContainerHandler(
		self.hostnameOverride, name, self.clusterName, self.quotadiskpath, self.cgroupSubsystems, self.machineInfoFactory, self.fsInfo, self.watcher, rootFs, self.ignoreMetrics)
}
...
func Register(hostnameOverride, clusterName, quotadiskpath string, machineInfoFactory info.MachineInfoFactory, fsInfo fs.FsInfo, ignoreMetrics map[container.MetricKind]struct{}) error {
